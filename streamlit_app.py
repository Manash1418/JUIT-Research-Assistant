...
if uploaded_file and process_btn:
    if not api_key:
        st.error("⚠️ **API Key Missing:** GEMINI_API_KEY is not configured in Streamlit Cloud Secrets or environment variables.\n\n"
                 "👉 **Fix:** In Streamlit Cloud, go to **App Settings ➔ Secrets** and add:\n"
                ...
                try:
                    ...
                    if "401" in err_msg or "UNAUTHENTICATED" in err_msg or "API_KEY_INVALID" in err_msg or "invalid authentication" in err_msg.lower():
                        st.error("🔑 **Authentication Failed:** The provided Gemini API Key is invalid or rejected by Google.\n\n"
                                 "👉 **Fix:** Update `GEMINI_API_KEY` in Streamlit Cloud Secrets with a valid key from [Google AI Studio](https://aistudio.google.com/app/apikey).")
